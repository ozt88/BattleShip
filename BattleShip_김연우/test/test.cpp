...
GameManager::GameManager()
	:m_GameState( WAIT ) , m_SystemState( INIT ) ,
	m_GameCount( 0 ) , m_NumOfTurn( 0 ) , m_Total( 0 ) , m_MyName( L"±èÂ¯½Ü" )
{
	m_Network = new Network();
	m_Player1 = new Player();
	m_Player1->SetName( L"Player1" );
...
void GameManager::End( const std::wstring& winner )
{
	...
	{
		//¼öÁ¤ÇØ¾ßµÊ
		m_GameScene->MakeEndScene( true , m_GameCount , m_Total );
...
void GameManager::End()
{
	...
	m_GameScene->MakeEndScene( gameResult.isWinner , m_WinNum , gameResult.turns );
}
...
void GameManager::Exit()
{
	...
	m_GameScene->MakeEndScene( L"total" , m_WinNum , ( int )average );
	SetGameState( GAME_END );
}
